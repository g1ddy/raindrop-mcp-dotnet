...
## 2024-05-29 - Missing Input Validation on TagRenameRequest Replace Property
**Vulnerability:** The `TagRenameRequest` DTO lacked a `[MaxLength]` validation attribute on the `Replace` string property. This property accepts unbounded arbitrary user input, creating a potential vector for Denial of Service (DoS) attacks via excessive memory consumption during processing or serialization, and risks downstream database constraint violations.
**Learning:** Shared request models and DTOs handling arbitrary text input MUST define explicit constraints (e.g., `[MaxLength]`) to ensure data bounds are strictly enforced before processing. This is consistent with other unbounded string fields in the application.
**Prevention:** Always verify and apply the `[MaxLength(Raindrop.MaxTextFieldLength)]` constraint to any user-provided unbounded text field within DTOs to enforce safe data boundaries.
