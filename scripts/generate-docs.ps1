...
$toolMethods = [Mcp.Common.RaindropToolBase].Assembly.GetTypes() `
    ...
    | Where-Object { $_.GetCustomAttributes([Mcp.Common.ToolAttribute], $false).Length -gt 0 }
...
foreach ($method in $toolMethods) {
    $toolAttribute = $method.GetCustomAttributes([Mcp.Common.ToolAttribute], $false)[0]
